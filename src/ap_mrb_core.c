...
void ap_mrb_raise_error(mrb_state *mrb, mrb_value obj, mod_mruby_code_t *code)
{
  ...
  if (mrb_type(obj) == MRB_TT_STRING) {
    ...
    ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, "%s ERROR %s: mrb_run failed: [TYPE: %s] [CACHED: %s] mruby raise: %s",
...
static mrb_value ap_mrb_errlogger(mrb_state *mrb, mrb_value str)
{
  ...
  if (argc != 2) {
    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, "%s ERROR %s: argument is not 2", MODULE_NAME, __func__);
    return str;
  }

  ap_log_error(APLOG_MARK, mrb_fixnum(argv[0]), 0, (ap_mrb_get_request())->server, "%s", mrb_str_to_cstr(mrb, argv[1]));
...
static mrb_value ap_mrb_syslogger(mrb_state *mrb, mrb_value str)
{
  ...
  return str;
#else
  ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, "%s ERROR %s: syslog was not supported", MODULE_NAME, __func__);
...
void ap_mruby_core_init(mrb_state *mrb, struct RClass *class_core)
{
  ...
  mrb_define_const(mrb, class_core, "HTTP_INTERNAL_SERVER_ERROR", mrb_fixnum_value(HTTP_INTERNAL_SERVER_ERROR));
  mrb_define_const(mrb, class_core, "HTTP_NOT_IMPLEMENTED", mrb_fixnum_value(HTTP_NOT_IMPLEMENTED));
  mrb_define_const(mrb, class_core, "HTTP_BAD_GATEWAY", mrb_fixnum_value(HTTP_BAD_GATEWAY));
  mrb_define_const(mrb, class_core, "HTTP_VARIANT_ALSO_VARIES", mrb_fixnum_value(HTTP_VARIANT_ALSO_VARIES));
